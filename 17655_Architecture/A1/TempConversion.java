...
public class TempConversion extends FilterFramework
{
	public void run()
    {
		/************************************************************************************
		...
		**************************************************************/
		...
		System.out.print( "\n" + this.getName() + "::A1.TempConversion Reading ");
		...
		while (true)
		{
			try
			{
				...
				*****************************************************************************/
				...
				if (id == 0) {
					...
					/****************************************************************************
					// ID 4 means that it is temperature data, so, next 8 bytes are going to be the
					// temperature, read them, convert the value to celsius and then finally convert
					// it to bytes and write it out to the filter's output along with the corresponding
					// ID.
					*****************************************************************************/

					if (id == 4) {
						// Write the ID of temperature to the output port
						ByteBuffer buffer = ByteBuffer.allocate(IdLength); // Create the ByteBuffer of legth 4
						bytesToBeWritten = buffer.putInt(id).array(); // Create the array from ByteBuffer
						...
						// Convert the temperature to the Bytes and write it to the filter's output port
						buffer = ByteBuffer.allocate(MeasurementLength); // Create the ByteBuffer of legth 8
						bytesToBeWritten = buffer.putLong(Double.doubleToLongBits(celsius)).array(); // Create the array from ByteBuffer
			...
			********************************************************************************/
			...
			{
				ClosePorts();
				System.out.print( "\n" + this.getName() + "::A1.TempConversion Exiting; bytes read: " + bytesread );
